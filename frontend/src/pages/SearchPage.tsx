...
function SearchPage() {
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      ...
      const friendsIds = friendsResponse.friends.reduce((prev: string[] ,curr: User)=>{
        return [...prev, curr.id]
    },[])
      ...
      <section className=" min-h-screen mx-50 lg:mx-72 ">
        <div>
          <form className="flex flex-row max-w-3xl w-full mt-5" onSubmit={handleSearch} >
            <input type="text" placeholder="John Doe" className="form-input text-my-darker" 
            onChange={e => setSearchState(e.target.value)}></input>
            <button type="submit" className="btn small bg-my-purple ml-5 text-xs">Search</button>
          </form>
        </div>
          <div>
            {usersFound && usersFound.length > 0 ? (
              usersFound.map((user, index) => (
                <FoundUser user={user[0]} key={String(index)} isFriend={isFriend(usersFriends, user[0])} />
              ))
            ) : (
              <p className="text-lg">Enter your friend's name in the field above.</p>
            )}
  </div>
