...
const doesNotificationRepeat = (
  notification: RoomNotification,
  notificationArray: RoomNotification[],
) => {
  const doesExist = notificationArray.find(
...
export default (state = [], action: NotificationsAction) => {
  if (action.type === "SET_NOTIFICATIONS") {
    return action.payload.notifications!.reduce(
      (acum: RoomNotification[], item: RoomNotification) => {
    ...
    return state.filter(
      (notification: RoomNotification) =>
        notification.roomId !== action.payload.id,
    );
  } else {
    return state;
  }
